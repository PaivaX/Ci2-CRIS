class VacanciesController < ApplicationController
  ...
  def create
    ...
    if(params[:researcher_ids].length != 0)
      params[:researcher_ids].each do |id|
      unless id.to_s.strip.empty?
        @vacancy.researchers << Researcher.find(id)
      end
    end
    end
  ...
  private
    ...
    def vacancy_params
      params.require(:vacancy).permit(:reference, :position, :requirement, :deadline, :link, :euraxess, :project_id, :researcher_id)
    end
end
